...
#
# bash.showRemoteRepoName
# -----------------------
# Option | Description
# ------ | --------------
# true   | _Default_. Remote Repo Name will be displayed or a message indicating
#          there is no remote repo defined.
# false  | No Remote Repo Name will be displayed.
#

###############################################################################
#echo "posh-git-prompt.sh has been sourced..."
...
__posh_git_echo () {
    ...
    local StashForegroundColor=$(__posh_color '\e[1;34m') # blue like orchid
    local StashBackgroundColor=

    local RemoteRepoForegroundColor=$(__posh_color '\e[1;31m') # orange
    local RemoteRepoBackgroundColor=

    #local StashText='$'
    local StashText=$(git_stash_size)
    ...
    local ShowRemoteRepoName=`git config --bool bash.showRemoteRepoName`
    case "$ShowRemoteRepoName" in
        true)  ShowRemoteRepoName=true ;;
        false) ShowRemoteRepoName=false ;;
        *)     ShowRemoteRepoName=true ;;
    esac
    ...
    if $ShowRemoteRepoName; then
        local RemoteRepoIdent=$(git_remote_repo_name)
        gitstring+="$RemoteRepoBackgroundColor$RemoteRepoForegroundColor$RemoteRepoIdent"
    fi
...
#------------------------------------------------------------------------------
# variable=$(git_remote_repo_name)
function git_remote_repo_name {
    remote_format="  << No Remote Repo Defined >> "
